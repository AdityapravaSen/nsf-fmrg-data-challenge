"""23_phase3_mlp_baseline.py

Final Phase III baseline neural-network experiment.

This script trains a simple feed-forward MLP regressor to predict the frozen PCA
shape targets (PC1--PC5) from flattened Phase III feature windows. Unlike the
LSTM baseline, this experiment intentionally uses the same flattened feature
representation as the classical Linear Regression, Ridge Regression, and Random
Forest baselines.
...
LSTM_VALIDATION_BASELINE = {
    "thermal_only": {"mae": 3.093022, "rmse": 3.289154, "median_absolute_error": 3.125284, "r2": -4.971182},
    "sem_only": {"mae": 1.441860, "rmse": 1.792883, "median_absolute_error": 1.183707, "r2": -0.978544},
    "thermal_sem": {"mae": 2.015820, "rmse": 2.293431, "median_absolute_error": 1.916156, "r2": -1.907598},
}
...
class MLPRegressor(nn.Module):
    """Small feed-forward MLP regressor for flattened feature windows."""
...
def flatten_sequence_features(x_seq: np.ndarray) -> np.ndarray:
    """Flatten sequence windows for the MLP baseline."""
...
def build_phase3_datasets(feature_columns: list[str]) -> tuple[Phase3TorchDataset, Phase3TorchDataset, object, object]:
    ...
    train_dataset = Phase3TorchDataset(x_train_flat, y_train, train_target_meta)
    val_dataset = Phase3TorchDataset(x_val_flat, y_val, val_target_meta)
...
def train_mlp(model: nn.Module, train_loader: DataLoader, device: torch.device) -> list[float]:
    """Train the MLP baseline with a fixed simple training loop."""
...
def describe_change(candidate_value: float, baseline_value: float, lower_is_better: bool = True, tolerance: float = 1.0e-6) -> str:
    """Describe MLP performance relative to a prior baseline."""
...
def run_feature_group_experiment(group_name: str, feature_columns: list[str], device: torch.device) -> dict[str, object]:
    """Run one MLP baseline for one feature group."""
    ...
    model = MLPRegressor(input_size=input_size, output_size=output_size).to(device)

    print("Shapes")
    print(f"  X_train_flat: {train_dataset.X.shape}")
    print(f"  Y_train:      {train_dataset.Y.shape}")
    print(f"  X_val_flat:   {val_dataset.X.shape}")
    print(f"  Y_val:        {val_dataset.Y.shape}")
    print("Model")
    print(f"  input_size={input_size}, hidden_sizes=({HIDDEN_SIZE_1}, {HIDDEN_SIZE_2}), output_size={output_size}")
    print(f"  dropout={DROPOUT}, epochs={EPOCHS}, batch_size={BATCH_SIZE}, learning_rate={LEARNING_RATE}")

    losses = train_mlp(model, train_loader, device)
...
def print_comparison_summary(results: list[dict[str, object]]) -> None:
    ...
    print("\n" + "=" * 78)
    print("MLP baseline comparison: validation split")
    print("Target: PCA shape (PC1--PC5)")
    print("Split: train Tracks 8+10, validation Track 14")
    print(f"Architecture: flattened input -> {HIDDEN_SIZE_1} -> {HIDDEN_SIZE_2} -> 5")
    ...
    print("Feature group                 vs Linear MAE    vs Ridge MAE     vs RF MAE        vs LSTM MAE      vs RF R2        vs LSTM R2")
    print("-" * 126)
    for result in results:
        group_name = result["group_name"]
        mlp = result["val_metrics"]["overall"]
        ...
        lstm = LSTM_VALIDATION_BASELINE[group_name]
        print(
            f"{group_name:<28} "
            f"{describe_change(mlp['mae'], linear['mae'], lower_is_better=True):<16} "
            f"{describe_change(mlp['mae'], ridge['mae'], lower_is_better=True):<16} "
            f"{describe_change(mlp['mae'], forest['mae'], lower_is_better=True):<16} "
            f"{describe_change(mlp['mae'], lstm['mae'], lower_is_better=True):<16} "
            f"{describe_change(mlp['r2'], forest['r2'], lower_is_better=False):<16} "
            f"{describe_change(mlp['r2'], lstm['r2'], lower_is_better=False):<16}"
        )


def main() -> int:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Phase III baseline experiment: flattened-window MLP Regression")
    ...
    print("Sequence handling: flattened X_seq, matching classical baselines")
    ...
    print("\nPhase III MLP baseline experiment complete: PASS")
    return 0
